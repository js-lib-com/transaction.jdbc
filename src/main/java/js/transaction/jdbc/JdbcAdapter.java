...
final class JdbcAdapter implements Configurable
{
  ...
  /**
   * Thread local storage. Use this thread specific storage to cache the database handler attached to current
   * transaction.
   */
  ...
  /**
   * Create a database handler. Create a new {@link Transaction} and store it on current thread so that it can be
   ...
   * 
   * @param readOnly if this flag is true create a read-only handler, that is, supporting only database select
   *          operations.
